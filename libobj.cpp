...
glmReadMTL(GLMmodel* model, char* name)
{
    ...
    for (i = 0; i < nummaterials; i++) {
        ...
        model->materials[i].texture = (char*) "NO";
    }

    model->materials[0].name      = strdup("default");
    model->materials[0].texture   = strdup("NO");
...
GLvoid glmLoadInVBO(GLMmodel* model)
{
    ...
    while (group)
    {
        ...
        // Reserve memory space for VBO texcoords
        glm::vec2* textcoords = (glm::vec2*)malloc(sizeof(glm::vec2) * numPointsInVBO);
        ...
        for (unsigned int i = 0; i < group->numtriangles; i++)
        {
            ...
            for (int j = 0; j < 3; j++)
            {
                ...
                textcoords[counter] = glm::vec2(x, -y);
                counter++;
            }
        }
        // Calculate sizes...
        ...
        int sizeTexCoords   = sizeof(GLfloat) * 2 * numPointsInVBO;
...
//        printf("tex %d\n" , texcoordBuffer);
        ...
        glBufferData( GL_ARRAY_BUFFER, sizeTexCoords, textcoords, GL_STATIC_DRAW );
        ...
        group->texcoordBuffer   = texcoordBuffer;
        group = group->next;
...
GLvoid
glmDrawVBO(GLMmodel* model, Texture* pTextures, GLuint program)
{
    GLMmaterial*    material;
    GLMgroup*       group;
    int count = 0;
...
//            continue;
        ...
        if(strcmp(material->texture, "NO"))
        {
            pTextures[count].Bind(0);
        }
        else
        {
        ...
        glUniform1i ( glGetUniformLocation(program, "gSampler"),        0);
        ...
        glEnableVertexAttribArray(2); // TODO this 0 has to change
        glBindBuffer( GL_ARRAY_BUFFER, group->texcoordBuffer );
...
bool LoadTextures(GLMmodel* model, Texture *pTextures, char const* path_to_textures)
{
    GLMmaterial*    material;
    GLMgroup*       group;
    int count = 0;
    ...
    while(group)
    {
        ...
        /* ============================================================================================= */
        std::string tex_prefix = path_to_textures;
        std::string tex_path = tex_prefix + material->texture;
        pTextures[count] = Texture(GL_TEXTURE_2D, tex_path);
        if (strcmp(material->texture, "NO"))
        {
            printf("Load texture: %s \n", material->texture);
            if(!pTextures[count].Load())
            {
                material->texture = (char*) "NO";
                //return;
            }
        }
        /* ============================================================================================= */
...
GLvoid
glmDisplayVBOMaterials(GLMmodel* model)
{
    GLMmaterial*    material;
    GLMgroup*       group;
